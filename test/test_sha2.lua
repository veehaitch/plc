...
local function test_sha256_nist_short()
	local test_cases = {
		{
			L = 0,
			Msg = "",
	...
	for i, test_case in ipairs(test_cases) do
		local msg = xts(test_case.Msg)
		local actual = stx(sha2.sha256(msg))
		local expected = test_case.MD

		if expected ~= actual then
			print(string.format("FAILED: test%s (L=%s)\n  Expected: %s\n  Actual  : %s", i, test_case.L, expected, actual))
			passed = false
		else
			print(string.format("PASSED: test%s (L=%s)", i, test_case.L))
		end
	end

	assert(passed, "Some tests failed")
end
...
test_sha256_nist_short()

print("test_sha256_nist_short: ok")